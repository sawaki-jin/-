<?php 
    ...
    try {
        ...
        for ($yr = $minyear; $yr <= $maxyear; $yr++) {
            $data[$yr] = [
                'shokuzaihi' => array_fill(1, 12, 0),
                'jinkenhi' => array_fill(1, 12, 0),
                'suidoukounetuhi' => array_fill(1, 12, 0),
                'hanbaisokusinhi' => array_fill(1, 12, 0),
                'housouhi' => array_fill(1, 12, 0),
                'hendouhisonota' => array_fill(1, 12, 0),
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $data[$year]['shokuzaihi'][$month] += $row['shokuzaihi'];
            $data[$year]['jinkenhi'][$month] += $row['jinkenhi'];
            $data[$year]['suidoukounetuhi'][$month] += $row['suidoukounetuhi'];
            $data[$year]['hanbaisokusinhi'][$month] += $row['hanbaisokusinhi'];
            $data[$year]['housouhi'][$month] += $row['housouhi'];
            $data[$year]['hendouhisonota'][$month] += $row['hendouhisonota'];

            // 合計を計算
            $data[$year]['total'][$month] = $data[$year]['shokuzaihi'][$month] + 
                                            $data[$year]['jinkenhi'][$month] +
                                            $data[$year]['suidoukounetuhi'][$month] +
                                            $data[$year]['hanbaisokusinhi'][$month] +
                                            $data[$year]['housouhi'][$month] +
                                            $data[$year]['hendouhisonota'][$month];
        }
...
<body>
    ...
    <div class="container mt-5">
        <h2><i class="fas fa-money-bill-wave"></i> 変動費</h2>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php foreach ($data[$yr] as $key => $values) : ?>
                                <?php if ($key !== 'total') : ?>
                                <tr>
                                    <td><?= str_replace(['shokuzaihi', 'jinkenhi', 'suidoukounetuhi', 'hanbaisokusinhi', 'housouhi', 'hendouhisonota'], 
                                                       ['食材費', '人件費', '水道光熱費', '販売促進費', '包装費', 'その他'], $key) ?></td>
                                    <?php for ($i = 1; $i <= 12; $i++) : ?>
                                        <td><?= $values[$i] ?></td>
                                    <?php endfor; ?>
                                </tr>
